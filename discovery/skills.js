...
export async function craftRecipe(bot, itemName, num=1) {
    /**
     * レシピからアイテムを作成します。複数のアイテムを作成する可能性があります。
     * @param {MinecraftBot} bot Minecraftボットへの参照
     * @param {string} itemName 作成するアイテムの名前
     * @returns {Promise<boolean>} レシピの作成に成功した場合はtrue、失敗した場合はfalse
     * @example
    ...
    if (mc.getItemCraftingRecipes(itemName).length == 0) {
        log(bot, `${itemName}はアイテムではないか、クラフトレシピがありません！`);
        return false;
    }
    ...
    placeTable: if (!recipes || recipes.length === 0) {
        ...
        if(!recipes || recipes.length === 0) break placeTable; //必要な資源がない場合は作業台に行く必要はない
        ...
        if (craftingTable === null){
            ...
            if (hasTable) {
                ...
                log(bot, `${itemName}のクラフトには作業台が必要です。`)
                return false;
            }
        }
        else {
    ...
    if (!recipes || recipes.length === 0) {
        log(bot, `${itemName}をクラフトするための資源が不足しています。必要なもの: ${Object.entries(mc.getItemCraftingRecipes(itemName)[0][0]).map(([key, value]) => `${key}: ${value}`).join(', ')}.`);
        if (placedTable) {
    ...
    console.log('クラフト中...');
    //レシピを`num`回使用するのに十分なアイテムがあるか確認
    const inventory = world.getInventoryCounts(bot); //エージェントのインベントリ内のアイテム
    const requiredIngredients = mc.ingredientsFromPrismarineRecipe(recipe); //レシピを1回使用するのに必要なアイテム
    const craftLimit = mc.calculateLimitingResource(inventory, requiredIngredients);
    
    await bot.craft(recipe, Math.min(craftLimit.num, num), craftingTable);
    if(craftLimit.num<num) log(bot, `${craftLimit.limitingResource}が足りないため${num}個ではなく${craftLimit.num}個しかクラフトできませんでした。現在${world.getInventoryCounts(bot)[itemName]}個の${itemName}があります。`);
    else log(bot, `${itemName}のクラフトに成功しました。現在${world.getInventoryCounts(bot)[itemName]}個の${itemName}があります。`);
    if (placedTable) {
        await collectBlock(bot, 'crafting_table', 1);
    }

    //クラフトした防具を装備する
    //インベントリ全体をチェックするより効率的な方法があるかもしれませんが、mineflayer-armor-managerが提供しているのはこれだけです :P
